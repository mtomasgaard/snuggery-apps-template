"""Builds dist/besseggen.zip for importing into Snuggery: the app files inside one wrapping folder,
...
SKIP_FILES = {'README.md', 'HANDOFF.md', 'NOTES.md', '.gitignore', '.gitattributes', '.DS_Store'}
...
out = os.path.join(out_dir, 'besseggen.zip')
...
for dp, dn, fn in os.walk(APP):
    ...
    for f in fn:
        ...
        if rel in SKIP_FILES or os.path.islink(os.path.join(dp, f)):
            continue
        files.append(rel)
files.sort()
...
assert os.path.isdir(os.path.join(APP, 'data')), 'data/ missing — run build_all.sh'
...
assert not offenders, ('a URL in the app\'s own code — Snuggery blocks every network request, so '
                       'this must go (licence links belong in CREDITS.txt, README.md or '
                       'data/about.json):\n  ' + '\n  '.join(offenders[:10]))
...
with zipfile.ZipFile(out, 'w', zipfile.ZIP_DEFLATED, compresslevel=9) as z:
    for f in files:
        z.write(os.path.join(APP, f), 'besseggen/' + f.replace(os.sep, '/'))

print(f'{out}: {len(files)} files, {sum(sizes.values())/2**20:.1f} MB unpacked, '
      f'{os.path.getsize(out)/2**20:.1f} MB zipped')
